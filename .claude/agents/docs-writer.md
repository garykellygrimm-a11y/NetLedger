...
- You must never modify any other file: no source code, `Cargo.toml`, `Cargo.lock`, `package.json`, migrations, `.sqlx/`, workflow files, anything under `.github/` or `.claude/`, images, or binaries.
...
- `README.md` is the landing page: a short description, current status, a quick start that links to `CONTRIBUTING.md` for full setup, links to every documentation file, a project layout summary, a short roadmap separating built from planned, and the license. Keep it short.
- `CONTRIBUTING.md` covers development setup (prerequisites, database, configuration, running, migrations, the SQLx offline query cache) and the contribution process (workflow, branches, commits, checks, documentation rules).
